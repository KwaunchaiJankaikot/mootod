...
export default {
  ...
  mounted() {
    ...
    // ตรวจสอบสถานะการแจ้งเตือนใน localStorage
    if (localStorage.getItem('menuAdded') === 'true') {
      alert("เมนูถูกเพิ่มสำเร็จ!");
      localStorage.removeItem('menuAdded');  // ลบสถานะหลังจากแสดงการแจ้งเตือน
    }
  },
  methods: {
    fetchMenus() {
      axios
        ...
        .catch((error) => {
          console.error("เกิดข้อผิดพลาด:", error.response ? error.response.data : error);
    ...
    uploadMenu() {
      ...
      axios
        ...
        .then(() => {
          // ตั้งค่าสถานะใน localStorage เพื่อแจ้งเตือน
          localStorage.setItem('menuAdded', 'true');
