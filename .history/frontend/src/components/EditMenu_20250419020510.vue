...
updateMenu() {
  ...
  axios
    ...
    .then(response => {
      console.log(response.data);  // เพิ่มการแสดงข้อมูลการตอบกลับจาก API
      alert("อัปเดตเมนูสำเร็จ!");
      this.$router.push("/menu-list");
    })
    .catch(error => {
      console.error("เกิดข้อผิดพลาดในการอัปเดตเมนู:", error); // เพิ่มการแสดงข้อผิดพลาด
      alert("เกิดข้อผิดพลาดในการอัปเดตเมนู");
      if (error.response) {
        // ถ้า API ส่งข้อมูลผิดพลาด
        console.error("ข้อผิดพลาดจาก API:", error.response.data);
      }
    });
},
