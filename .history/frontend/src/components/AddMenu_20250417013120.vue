...
export default {
  data() {
    return {
      ...
      imageName: "",
      fetchError: false,  // เพิ่มสถานะสำหรับการตรวจสอบว่าเกิดข้อผิดพลาดในการดึงข้อมูลหรือไม่
    };
  },
  mounted() {
    this.fetchMenus();
    ...
    fetchMenus() {
      axios
        ...
        .then((response) => {
          ...
          this.fetchError = false;  // ถ้าดึงข้อมูลสำเร็จให้รีเซ็ตสถานะข้อผิดพลาด
        })
        .catch((error) => {
          console.error(
            "เกิดข้อผิดพลาด:",
          ...
          if (!this.fetchError) {
            ...
            this.fetchError = true;  // ตั้งค่าผลลัพธ์ให้เป็นข้อผิดพลาด เพื่อไม่ให้แสดงซ้ำ
          }
        });
    },
    onImageChange(event) {
