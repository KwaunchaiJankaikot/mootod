...
mounted() {
  this.fetchMenu();  // เรียกฟังก์ชันดึงข้อมูลเมนู
},

methods: {
  fetchMenu() {
    const menuId = this.$route.params.id;  // ดึง ID เมนูจาก URL
    console.log('Fetching menu with ID:', menuId); // ตรวจสอบว่ามีค่า menuId หรือไม่
    axios
      .get(`http://localhost:5000/menus/${menuId}`)
      .then((response) => {
        const menu = response.data;
        console.log('Fetched menu data:', menu); // ตรวจสอบข้อมูลที่ได้มา
        if (menu) {
      ...
      .catch((error) => {
        console.error("ไม่สามารถดึงข้อมูลเมนูได้:", error); // ตรวจสอบ error ที่เกิดขึ้น
      });
  },
