...
methods: {
  fetchMenu() {
    const menuId = this.$route.params.menuId;  // ดึง ID จาก URL
    axios.get(`http://localhost:5000/menus/${menuId}`)
      .then(response => {
  ...
  updateMenu() {
    ...
    if (this.menu.image) {
      ...
      formData.append("image_url", this.menu.image_url);  // ใช้ image_url หากไม่มีการอัปโหลดรูปใหม่
    }

    axios.put(`http://localhost:5000/menus/${this.menu.id}`, formData)
      .then(() => {
