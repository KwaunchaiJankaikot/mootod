    ...
    <div class="form-card">
      ...
      <form @submit.prevent="updateMenu">
        ...
        <div v-if="menu.image_url && !menu.image" class="existing-image">
          <p>รูปภาพปัจจุบัน:</p>
          <img :src="menu.image_url" alt="Current Image" class="current-image" />
          <!-- ปุ่มลบรูปภาพ -->
          <button type="button" @click="deleteImage" class="delete-image-btn">ลบรูปภาพ</button>
        </div>
...
export default {
  ...
  methods: {
    // ฟังก์ชันดึงข้อมูลเมนูจาก API โดยใช้ ID
    fetchMenu() {
      ...
      axios
        ...
        .then(response => {
          ...
          this.menu.image_url = menu.image_url; // ดึง image_url ที่มีในฐานข้อมูล
        })
        .catch(error => {
...
.delete-image-btn,.submit-button {
  gap: 0px 100px;
}
</style>
