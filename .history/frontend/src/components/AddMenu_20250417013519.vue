...
export default {
  data() {
    return {
      ...
      imageName: "",
      fetchError: false, // ใช้สถานะสำหรับการตรวจสอบข้อผิดพลาด
      submitError: false, // ใช้สถานะสำหรับการตรวจสอบข้อผิดพลาดจากการส่งข้อมูล
    };
  },
  mounted() {
    this.fetchMenus();
    ...
    fetchMenus() {
      axios
        ...
        .then((response) => {
          ...
          this.fetchError = false; // ถ้าดึงข้อมูลได้สำเร็จ ให้รีเซ็ตสถานะ
        })
        .catch((error) => {
          console.error("เกิดข้อผิดพลาด:", error);
          if (!this.fetchError) {
            alert("เกิดข้อผิดพลาดในการดึงข้อมูลเมนู");
            this.fetchError = true; // ตั้งค่าผลลัพธ์ให้เป็นข้อผิดพลาด
          }
        });
    },
    onImageChange(event) {
    ...
    uploadMenu() {
      ...
      axios
        ...
        .then(() => {
          ...
          this.menu.image = null;
          this.imageName = "";
          this.fetchMenus(); // ดึงข้อมูลเมนูใหม่หลังจากเพิ่ม
        })
        .catch((error) => {
