...
client.connect()
  ...
  .catch(err => {
    ...
    process.exit(1); // หยุดการทำงานหากไม่สามารถเชื่อมต่อกับฐานข้อมูล
  });
...
// PUT route สำหรับอัปเดตเมนู
app.put('/menus/:id', upload.single('image'), (req, res) => {
  const { name, description, price } = req.body;
  const { filename } = req.file;
  const imageUrl = filename ? `http://localhost:5000/images/${filename}` : null;
  ...
  const query = `
    ...
    WHERE id = $5
  `;
  const values = [name, description, price, imageUrl, menuId];
  ...
  client.query(query, values, (err, result) => {
    ...
    res.status(200).json({ message: 'เมนูถูกอัปเดตสำเร็จ' });
  });
});
