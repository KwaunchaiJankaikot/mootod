...
app.put('/menus/:id', upload.single('image'), (req, res) => {
  ...
  const { name, description, price } = req.body;
  const { filename } = req.file;
  const imageUrl = filename ? `http://localhost:5000/images/${filename}` : req.body.image_url;
  ...
  client.query(
    ...
    (err, result) => {
      ...
      res.status(200).json({ message: 'เมนูถูกอัปเดตสำเร็จ', imageUrl: imageUrl });
    }
  );
});
...
app.put('/menus/:id', upload.single('image'), (req, res) => {
  ...
  let imageUrl = req.body.image_url; // ใช้ URL รูปภาพเดิมเป็นค่าเริ่มต้น
  if (req.file) {
