...
app.put('/menus/:id', (req, res) => {
  ...
  // ตรวจสอบว่า body มาจากฟอร์มหรือไม่
  ...
  client.query(
    ...
    (err, result) => {
      ...
      console.log('Existing Menu:', result.rows);  // แสดงข้อมูลเมนูที่ดึงออกมา
      ...
      console.log('Files:', req.files);  // ตรวจสอบว่ามีไฟล์หรือไม่
      if (req.files && req.files.image) {
        const filename = req.files.image[0].filename;
        imageUrl = `http://localhost:5000/images/${filename}`;
      }

      console.log('Final Image URL:', imageUrl); // แสดง URL รูปภาพที่จะใช้
      ...
      client.query(
        ...
        (err, updateResult) => {
          ...
          console.log('Update Result:', updateResult.rows); // แสดงข้อมูลเมนูที่อัปเดต
